...
def exp_YJ_Covid(args):
    ...
    (train_loader, validation_loader, test_loader), (train_origin, validation_origin, test_origin), (train_indices, validation_indices, test_indices), (data_origin, date_all) = load_data(args)
    ...
    graph_lambda_0, graph_lambda_k, graph_lambda_method, = args.graph_lambda_0, args.graph_lambda_k, args.graph_lambda_method
...
def exp_dataforgood(args):
    ...
    for i_country in range(len(meta_data)):
        ...
        (train_loader, validation_loader, test_loader), (train_origin, validation_origin, test_origin), (train_indices, validation_indices, test_indices) = meta_data[country_name]

        logger.info(f"开始评估 {country_name}")

        # 记录实验参数
        result_paths.update({
            'model': os.path.join(args.result_dir, f"model_{country_code}_best.pth"),
            'model_latest': os.path.join(args.result_dir, f"model_{country_code}_latest.pth"),
            'csv': os.path.join(args.result_dir, f'results_{country_code}.csv'),
            'tensorboard': os.path.join(args.result_dir, f"tensorboard_{country_code}")
        })
        ...
        logger.info('-' * 20)
        logger.info(font_yellow("[最新]"))
        metrics_latest = eval_process(trained_model, criterion,
                    train_loader, validation_loader, test_loader,
                    args.ydays, case_normalize_ratio, device)
        
        logger.info('-' * 20)
        logger.info(font_yellow(f"[最小 val loss (epoch {epoch_best})]"))
        metrics_minvalloss = eval_process(result_paths['model'], criterion,
                    train_loader, validation_loader, test_loader,
                    args.ydays, case_normalize_ratio, device)
        
        writer.add_hparams(vars(args), {**metrics_latest, **metrics_minvalloss})
        writer.close()
...
def main():
    ...
    try:
        if args.dataset == 'YJ_COVID19': exp_YJ_Covid(args)
        elif args.dataset == 'dataforgood': exp_dataforgood(args)
        else: raise ValueError(f"数据集 {args.dataset} 不存在")
    finally:
