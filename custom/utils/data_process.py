...
def read_data(data_dir, start_date, end_date, x_days, y_days, case_normalize_ratio, text_normalize_ratio):
    ...
    data_origin = [
        np.empty((data_len, x_days, len(zones), len(zones))),        # mobility
        np.empty((data_len, x_days, len(zones), len(sym_names))),    # text
    ...
    for i in qbar1:
        ...
        for i_day, day in enumerate(features_days):
            ...
            for zone in mobility[day]:
                ...
                data_origin[0][i, i_day, zones.index(zones_from), zones.index(zones_to)] = mobility[day][zone]
            # text
            for zone in text[day]:
                for i_sym, sym in enumerate(sym_names):
                    data_origin[1][i, i_day, zones.index(zone), i_sym] = np.array([text[day][zone][sym]])
            # case x
            _dates = [date2str(i) for i in DateRange(
                str2date(day) - 20, str2date(day) + 1
                )]
            for zone in cases[day]:
                data_origin[2][i, i_day, zones.index(zone)] = \
        ...
        for i_day, day in enumerate(labels_days):
            ...
            for zone in cases[day]:
                data_origin[3][i, i_day, zones.index(zone)] = \
                    np.array([cases[d][zone] for d in _dates])
        data_origin[4][i] = i

    sum_over_zone_from = np.sum(data_origin[0], axis=-2)
    sum_over_zone_from_expand = np.expand_dims(sum_over_zone_from, axis=-2)
    data_origin[0] = data_origin[0] / sum_over_zone_from_expand
    data_origin = [torch.tensor(d) for d in data_origin]
    # data_origin[0] = data_origin[0] / data_origin[0].sum(-2).unsqueeze(-2) # 归一化 mobility，一个地区出发的素所有值之和为 1
    # 归一化 casex
