...
def print_err(args, results, _model):
    ...
    for result in results:
        ...
        for country in countries:
            ...
            s['minvalloss'][key][country] = dict(
                err_val=f"{err_test_minvalloss}",
                err_test=f"{err_test_minvalloss}",
                epoch=f"{epoch_minvalloss}"
            )
            s['latest'][key][country] = dict(
                err_val=f"{err_test_latest}",
                err_test=f"{err_test_latest}",
                epoch=f"{epoch_latest}"
            )
    _ = 1
    print(args.subdir, ','.join(countries))
    for k in s:
        print(k)
        print(' | '.join([' '.join(map(lambda c: c['err_test'], v.values())) for v in s[k].values()]))
        print(' | '.join([' '.join(map(lambda c: c['err_val'], v.values())) for v in s[k].values()]))
