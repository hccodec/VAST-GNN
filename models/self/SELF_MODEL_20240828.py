'''
该模型在 202407188 模型基础上，引入异质网络构建方法
...
class SelfModel(nn.Module):
    ...
    def forward(self, X, y, A, extra_info=None, idx=None):
        ...
        lstm_input = []
        adj_output = []
        for day in range(self.x_days):
            x = X[:, day, :, :]
            adj = A[:, day, :, :]
            x_extra_info = extra_info[:, day, :, :]

            if hasattr(self, 'graph_learner'):
                adj_hat = self.graph_learner(x_extra_info)
                z = self.graph_encoder(x_extra_info, adj_hat)
                adj_output.append(adj_hat.unsqueeze(1))
            else:
                z = self.graph_encoder(x_extra_info, adj)
