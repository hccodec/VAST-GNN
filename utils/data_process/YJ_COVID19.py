...
def read_data(data_dir,dataset, start_date, end_date, x_days, y_days, case_normalize_ratio, text_normalize_ratio):
...
def load_data(args):
    ...
    data_dir, dataset, case_normalize_ratio, text_normalize_ratio = args.data_dir, args.dataset, args.case_normalize_ratio, args.text_normalize_ratio
    ...
    if res is None:
        res = read_data(data_dir, dataset, start_date, end_date, x_days, y_days, case_normalize_ratio, text_normalize_ratio)
        with open(path, 'wb') as f:
            pickle.dump(res, f)
            logger.info(f'将数据文件保存至 {path}')

    train_loader, validation_loader, test_loader, train_origin, validation_origin, test_origin, train_indices, validation_indices, test_indices = split_dataset(args, *res)
    logger.info("数据准备完成，开始训练")

    return (train_loader, validation_loader, test_loader), (train_origin, validation_origin, test_origin), (train_indices, validation_indices, test_indices), res
