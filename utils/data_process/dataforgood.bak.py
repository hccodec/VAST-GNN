    ...
    for i_date in range(len(dates)):
        graph = pd.read_csv(
            os.path.join(
                ...
                country_names[i_country],
                "graphs",
                "{}_{}.csv".format(country_codes[i_country], dates[i_date]),
            ),
            header=None,
        )
        G = nx.DiGraph()
...
def split_dataset(args, 
    ...
    train_indices, val_indices, test_indices, get_y_indices = generate_indices(args, num_days)

    # extract formatted data with xdays/yays
    xdays, ydays = args.xdays, args.ydays
    X = torch.zeros_like(H).unsqueeze(1).repeat(1, xdays, 1, 1)
    A = torch.zeros_like(adjs).unsqueeze(1).repeat(1, xdays, 1, 1)
    Y = torch.zeros_like(y).unsqueeze(1).repeat(1, ydays, 1, 1)
    A_Y = torch.zeros_like(adjs).unsqueeze(1).repeat(1, ydays, 1, 1)

    for i_date in range(H.shape[0]):
        for i_xdays in range(xdays):
            _i_date = i_date + i_xdays - xdays + 1
            if _i_date < 0: _i_date = 0
            X[i_date, i_xdays] = H[_i_date]
            A[i_date, i_xdays] = adjs[_i_date]
        for i_ydays in range(ydays):
...
def generate_indices(args, num_days, mode = 2):
    ...
    train_indices, val_indices, test_indices = sorted(train_indices), sorted(val_indices), sorted(
        test_indices)
