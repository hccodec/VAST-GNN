...
def extract_from_logfile(logpath):
    ...
    with open(logpath, encoding="utf-8") as f:
        for line in f:
            if "Training complete. Evaluating" in line:
                capture = 8
...
def process_log_segment(lines):
    ...
    # 定义正则表达式来匹配日志中的信息
    pattern_country = re.compile(r"Training complete. Evaluating: (\w+)")
    ...
    pattern_latest_epoch = re.compile(r"\[Latest \(epoch (\d+)\)\]")
    pattern_min_val_epoch = re.compile(r"\[Min val loss \(epoch (\d+)\)\]")
